...
const JWT_EXPIRES_IN: JwtExpiry = (process.env.JWT_EXPIRES_IN || '1d') as JwtExpiry;
...
export const register = async (userData: IUserCreate): Promise<Omit<IUser, 'password'>> => {
	try {
		...
		authLogger.info('User registered successfully', { userId: user });

		// In a live, we would send a verification email here
...
function generateToken(dt: Record<any, any>, secret: string, expiresIn: JwtExpiry) {
	return jwt.sign(dt, secret, { expiresIn });
}
...
export const login = async (
	...
	try {
		...
		// Find user by email
		const user = await db<IUser>('users').where('email', credentials.email).first();

		if (!user) {
			throw new ApiError(401, 'Invalid credentials');
		}

		// Check if user is verified in live version
		// if (!user.is_verified) {
		...
		// Generate JWT token
		const token = generateToken({ id: user.id, email: user.email }, JWT_SECRET, JWT_EXPIRES_IN);
		...
		await db('refresh_tokens').insert({
			...
			token_family: crypto.randomUUID().toString(), // For refresh token rotation
			user_agent: req.headers['user-agent'],
			ip_address: req.ip,
		});

		// Update last login
		await db<IUser>('users').where('id', user.id).update({ last_login: new Date() });
...
export const refreshToken = async (
	...
	try {
		...
		// Verify the token
		const user = await db<IUser>('users').where('id', storedToken.user_id).first();
...
export const verifyEmail = async (token: string): Promise<void> => {
	try {
		...
		const user = await db<IUser>('users').where('verification_token', token).first();
		...
		await db<IUser>('users').where('id', user.id).update({
			is_verified: true,
...
export const changePassword = async (
	...
	try {
		...
		await db.transaction(async (trx) => {
			await trx<IUser>('users').where('id', userId).update({ password: hashedPassword });
